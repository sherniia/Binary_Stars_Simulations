...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov  7 19:41:36 2021
...
mass_star = (3/5)*1.989*10**30
mass_sun = (7/10)*1.989*10**30

b_star_ratio = mass_b/mass_star 
b_sun_ratio = mass_b/mass_sun

g_sun = 1.98279e-29*((7/10))*1.989e+30
g_star = 1.98279e-29*(6/10)*1.989e+30
...
b_peri_speed_1 = 8.3 # the calculated speed is 4.9



t = np.linspace(0.0, 5, num=5000 ) 
t_1 = t*(3.154*10**7)
...
plto1= plt.figure(1)
plt.plot(soln[:, 0], soln[:,2],'#DC143C')
plt.plot(soln[:, 4], soln[:,6], 'orange')
plt.plot(soln[:, 8], soln[:,10], '#3CB371')
...
#df1.to_csv('Stars&planet_sim5.csv', index = False)
#rad_vel_habitable = soln_ms

#np.savetxt("data_4_binary.csv", df, delimiter=",")

#df.to_csv('Sim5_20_years.csv', index = False)
